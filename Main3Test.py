...
#ALPHA_VANTAGE_API_KEY = "33RHR6A2Q6JXADMVA"

# Output file
OUTPUT_FILE = "STOCK_ANALYSIS_RESULTS_.txt"

# Implementation of critical functions from advanced_quant_functions_backup.py
# Using full implementations with external dependencies

# Force enable advanced functions
USE_BACKUP_FUNCTIONS = True
print("[INFO] Advanced quantitative functions are ENABLED using full implementations")
...
def calculate_sigma(data):
    """Function to calculate sigma using advanced implementation"""
    ...
    if isinstance(results, dict) and 'combined_sigma' in results:
        ...
        raise ValueError(f"Failed to get combined_sigma from advanced quantitative analysis. Results: {results}")

def get_sigma_recommendation(sigma, analysis_details):
    """Function to get recommendation from advanced implementation"""
    ...
    tail_risk_metrics = {
        ...
        'crash_risk_index': analysis_details.get('crash_risk_index', 0)
    }
    ...
    if isinstance(results, dict) and 'combined_recommendation' in results:
        ...
        raise ValueError(f"Failed to get recommendation from advanced functions. Results: {results}")

# Fallback implementation has been completely removed in favor of advanced functions

# Enhanced technical indicators with log returns mean reversion components
def calculate_technical_indicators(data):
    print(f"[DEBUG] Calculating advanced technical indicators on data with shape: {data.shape}")
    
    # Use advanced technical analysis exclusively
    ...
    # Run the advanced technical indicators
    print(f"[INFO] Using advanced technical indicators with data shape {df_advanced.shape}")
    ...
    if result is not None:
        ...
        raise ValueError("Advanced technical indicators calculation failed")
...
def detect_market_regime(data, n_regimes=3):
    ...
    try:
        ...
        raise ValueError(f"Market regime detection failed completely: {e}")


# Risk-Adjusted Metrics with log return improvements
def calculate_risk_adjusted_metrics(df, sigma):
    """Calculate risk-adjusted metrics using advanced implementation"""
    from advanced_quant_functions_backup import run_tail_risk_analysis
    ...
    # Run the comprehensive analysis function
    ...
    if isinstance(risk_results, dict) and 'tail_risk_metrics' in risk_results:
        print("[INFO] Using advanced tail risk analysis")
        ...
        raise ValueError(f"Unexpected results from run_tail_risk_analysis: {risk_results}")
...
# Advanced data preparation for LSTM prediction with comprehensive feature engineering
def prepare_lstm_data(data, time_steps=60):
    # Check if we have enough data
    if len(data) < time_steps + 10:
        raise ValueError(f"Not enough data for LSTM: {len(data)} < {time_steps + 10}")
    ...
    # Make sure we have log returns
    if 'log_returns' not in data.columns:
        data['log_returns'] = np.log(data['4. close'] / data['4. close'].shift(1))
        data['log_returns'] = data['log_returns'].fillna(0)
        print("[INFO] Generated log returns for LSTM features")
    
    features.append(data['log_returns'].values)
    print("[INFO] Using log returns for LSTM features")

    # Include volume with appropriate scaling
    if 'volume' in data.columns:
        # Log transform volume to reduce scale differences
        log_volume = np.log1p(data['volume'].values)
        features.append(log_volume)
    else:
        # If volume not available, create a dummy column with zeros
        dummy_volume = np.zeros(len(data))
        features.append(dummy_volume)
        print("[INFO] Volume not available, using zeros")

    # Make sure we have log volatility
    if 'log_volatility' not in data.columns:
        if 'volatility' not in data.columns:
            # Calculate rolling std of log returns as volatility
            data['volatility'] = data['log_returns'].rolling(window=20).std() * np.sqrt(252)
            data['volatility'] = data['volatility'].fillna(method='bfill').fillna(0)
        
        data['log_volatility'] = np.log1p(data['volatility'])
        print("[INFO] Generated log volatility for LSTM features")
    
    features.append(data['log_volatility'].values)

    # Include RSI - calculate if not available
    if 'RSI' not in data.columns:
        # Calculate RSI
        delta = data['log_returns']
    ...
    if 'MACD' not in data.columns:
        ...
        print("[INFO] Generated MACD for LSTM features")
    
    # Normalize MACD using tanh for -1 to 1 range
    normalized_macd = np.tanh(data['MACD'].values / 5)
    features.append(normalized_macd)
    
    # Generate log-based mean reversion indicators if not available
    if 'log_returns_zscore' not in data.columns:
        # Calculate z-score of log returns
        mean = data['log_returns'].rolling(window=60).mean()
        std = data['log_returns'].rolling(window=60).std().replace(0, np.finfo(float).eps)
        data['log_returns_zscore'] = (data['log_returns'] - mean) / std
        data['log_returns_zscore'] = data['log_returns_zscore'].fillna(0)
        print("[INFO] Generated log returns z-score for LSTM features")
    
    # Normalize with tanh to -1 to 1 range
    log_returns_z = np.tanh(data['log_returns_zscore'].values)
    features.append(log_returns_z)

    # Create mean reversion potential if not available
    if 'BB_pctB' not in data.columns:
        # Calculate Bollinger Bands
        data['SMA20'] = data['4. close'].rolling(window=20).mean().fillna(method='bfill')
        data['BB_std'] = data['4. close'].rolling(window=20).std().fillna(0)
        data['BB_upper'] = data['SMA20'] + (data['BB_std'] * 2)
        data['BB_lower'] = data['SMA20'] - (data['BB_std'] * 2)
        bb_range = data['BB_upper'] - data['BB_lower']
        data['BB_pctB'] = np.where(
            bb_range > 0,
            (data['4. close'] - data['BB_lower']) / bb_range,
            0.5
        )
        print("[INFO] Generated BB_pctB for LSTM features")
    
    features.append(data['BB_pctB'].values)

    # Create distance from SMA200 if not available
    if 'dist_from_SMA200' not in data.columns:
        data['SMA200'] = data['4. close'].rolling(window=200).mean()
        data['SMA200'] = data['SMA200'].fillna(method='bfill')
        data['dist_from_SMA200'] = (data['4. close'] / data['SMA200']) - 1
        print("[INFO] Generated dist_from_SMA200 for LSTM features")
...
def build_lstm_model(input_shape):
    # Sophisticated deep architecture for advanced prediction accuracy
    inputs = Input(shape=input_shape)

    # Deep LSTM layers for sequence processing
...
def predict_with_lstm(data):
    # Set a generous execution time for thorough training
    max_execution_time = 480  # 8 minutes max for comprehensive training
    start_time = time.time()

    # Require minimum data for reliable prediction
    if len(data) < 60:
        raise ValueError("Not enough data for LSTM model")

    # Use an optimal window for best context capture
    time_steps = 60  # Best prediction accuracy with this window

    # Prepare data with comprehensive feature engineering
    X, y, scaler = prepare_lstm_data(data, time_steps=time_steps)

    # Ensure we have sufficient processed data
    if len(X) < 8:
        raise ValueError(f"Not enough data after preparation: {len(X)}")

    # Build deep model
    model = build_lstm_model((X.shape[1], X.shape[2]))

    # Use extensive training data
    max_samples = 2000  # Large dataset for comprehensive training
    ...
    early_stop = EarlyStopping(monitor='loss', patience=8, verbose=0)  # Increased patience
    reduce_lr = ReduceLROnPlateau(monitor='loss', factor=0.5, patience=5, min_lr=0.00005)

    # Initial full-scale training
    model.fit(
        X_train, y_train,
    ...
    # First fine-tuning with reduced learning rate
    try:
    ...
    # Final fine-tuning with small batch size
    model.fit(
        X_train, y_train,
        epochs=20,
    ...
    # Advanced ensemble prediction approach
    num_pred_samples = min(15, len(X))  # Expanded ensemble
    predictions = []
    ...
    # Emphasize more recent predictions
    weights = np.linspace(1.0, 0.4, len(predictions))
    ...
    last_actual_weights = np.linspace(1.0, 0.4, len(last_actuals))
    last_actual_weights = last_actual_weights / np.sum(last_actual_weights)
    last_actual = np.sum(last_actuals * last_actual_weights)

    # Calculate prediction with safety for division
    epsilon = 1e-10  # Avoid division by zero
    predicted_volatility_change = abs((avg_prediction - last_actual) / (last_actual + epsilon))

    print(f"[DEBUG] LSTM prediction: {predicted_volatility_change}")

    # Return a scaled measure with appropriate bounds
...
class DQNAgent:
    ...
    def _build_model(self):
        ...
        model = Sequential([
            Dense(512, activation="relu", input_shape=(self.state_size,)),  # Enhanced model with more capacity
            BatchNormalization(),
            Dropout(0.3),
...
def get_dqn_recommendation(data):
    try:
        ...
        max_train_points = min(800, len(data) - (lookback + 1))  # Increased from 200

        # Use appropriate step size to get good coverage of data
        step_size = max(1, (len(data) - (lookback + 1)) // 800)  # Adjusted for more points
#cahnge to 500 if need be

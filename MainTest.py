...
# Instead of tensorflow.keras, use the standalone Keras
...
def run_bayesian_regime_analysis(df):
    """
    Perform Bayesian regime analysis to determine market state
    """
    try:
...
# NEW: Stock Price Prediction Functions
...
def create_prediction_plot(stock_data, prediction_data, symbol, plot_dir="prediction_plots"):
    ...
    try:
        ...
        if not os.path.exists(plot_dir):
            ...
            print(f"[INFO] Created directory for prediction plots: {plot_dir}")
    ...
    try:
        if not os.path.exists(OUTPUT_FILE):
            with open(OUTPUT_FILE, "w") as file:
                ...
                file.write("="*50 + "\n\n")
            return True
        return True  # File already exists, don't overwrite it
    except Exception as e:
